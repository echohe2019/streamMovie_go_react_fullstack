...
func AdminReviewUpdate(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if role != "ADMIN" {
			c.JSON(http.StatusBadRequest, gin.H{"error": "user must be part of ADMIN"})
			return
		}
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error updating movie", "details": err.Error()})
			return
		}
...
func GetRecommendedMovies(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		userId, err := utils.GdtUserIdFromContext(c)
		fmt.Printf(userId)
		if err != nil {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error getting favourite genres by userId", "details": err.Error()})
			return
		}
		err = godotenv.Load(".env")
		if err != nil {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error fetching recommended movies", "details": err.Error()})
			return
		}
		defer cursor.Close(ctx)
		var recommendedMovies []models.Movie
		if err := cursor.All(ctx, &recommendedMovies); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error decoding recommended movies", "details": err.Error()})
			return
		}
